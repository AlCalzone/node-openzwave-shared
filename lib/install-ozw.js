...
// Pin the OZW version to 1.5 until the wrapper is updated
const branch = "1.5";
var ozwSourceUrl = `https://github.com/OpenZWave/open-zwave/archive/V${branch}.zip`;
var gypOptions = [];
...
function copyFiles() {
  wrench.copyDirSyncRecursive(tempPath + `/ozw/open-zwave-${branch}/config`, installPath + '/config');
  wrench.copyDirSyncRecursive(tempPath + `/ozw/open-zwave-${branch}/cpp/src`,  installPath + '/include', { filter: /^.*\.(?!h)[^.]+$/ });
  wrench.copyDirSyncRecursive(tempPath + `/ozw/open-zwave-${branch}/build/Release`,  installPath + '/bin');
}
...
module.exports = function(opts) {
  ...
  download(ozwSourceUrl, tempPath + "/ozw.zip", function(err) {
    ...
    fs.createReadStream( tempPath + "/ozw.zip")
      ...
      .on('close', function() {
        var sources = wrench.readdirSyncRecursive(tempPath + `/ozw/open-zwave-${branch}/cpp`);
        ...
        fs.writeFile(tempPath + `/ozw/open-zwave-${branch}/binding.gyp`, JSON.stringify(ozwGyp, null, 4), function(err) {
            if(err) handleError;

            console.log('Building open zwave');
            process.chdir(tempPath + `/ozw/open-zwave-${branch}`);
